...
Grid::Grid(size_t dim_row, size_t dim_col) : rows(dim_row), cols(dim_col), matrix()
{
	...
	for(size_t i =0; i < cols; i++ ) {
		for(size_t j =0; j < rows; j++ ) {
			...
			gridCell_object.set_spin(random_spin);
...
int main ( int argc, char* argv[] ) {
	...
	// Begin Flags
    int Limit_Number_avalanches=100;
	size_t num_cols = 100;
	size_t num_rows = 100;
	...
	while(Avalanche_number < Limit_Number_avalanches) {
		...
		grid_object.add_cell_spin(random_col,random_row,1); // 501 -> 250  701->350
		//cout << ". After adding 1 spin=" <<  grid_object.get_cell(random_col,random_row).get_spin() << "\n";
		
		if (grid_object.get_cell(random_col,random_row).get_spin() <= 4) {
			goto label2;
		}
